...
{
    /* PATCHED for the SDR++ DRM module: upstream Dream leaves eDecError
       uninitialised, and never assigns it on the success path either, so the
       function returns a stack-garbage EDecError value. On most release
       builds this value happens to be non-zero, which AudioSourceDecoder reads
       as a decode error and replaces the audio with silence - the symptom is
       a perfectly locked stream (FAC/SDC OK, high MER) that nonetheless
       produces zero audio frames. Initialising to DECODER_ERROR_OK fixes it. */
    EDecError eDecError = CAudioCodec::DECODER_ERROR_OK;
