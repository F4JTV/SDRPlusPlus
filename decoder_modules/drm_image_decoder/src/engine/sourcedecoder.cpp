...
void sourceDecoder::loadParams(transportBlock *tbPtr,unsigned char paramID,int len)
{
  ...
  switch(paramID)
    ...
    case 12:
      ...
      // Propage le nom au suivi de progression (pour l'UI SDR++).
      {
          std::lock_guard<std::mutex> lk(drmprogress::nameMtx);
          drmprogress::objectName = tbPtr->fileName.toStdString();
      }
      QApplication::postEvent( dispatcherPtr, stce );  // Qt will delete it when done
      break;
    default:
...
void sourceDecoder::addDataSegment()
{
  ...
  // Mise à jour du suivi de progression (consommé par le wrapper SDR++).
  drmprogress::currentSegments = (int)tbPtr->segmentsReceived;
  drmprogress::totalSegments   = (int)tbPtr->totalSegments;
  drmprogress::bodySize        = (int)tbPtr->bodySize;
  drmprogress::segmentSize     = (int)tbPtr->defaultSegmentSize;
