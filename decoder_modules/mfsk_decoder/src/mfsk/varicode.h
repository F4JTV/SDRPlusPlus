#pragma once
// FLDIGI-faithful MFSK varicode (IZ8BLY), ported from fldigi src/mfsk/mfskvaricode.cxx (GPL).
#include <cstdint>
namespace mfsk {
...
static inline int mfsk_varidec(unsigned int symbol) {
    for (int i = 0; i < 256; i++) if (symbol == MFSK_VARIDECODE[i]) return i;
    return -1;
}
// FLDIGI naming: graydecode(d) = d ^ (d>>1)  (i.e. binary->gray, applied to the tone index)
static inline unsigned char mfsk_graydecode(unsigned char d) { return (unsigned char)(d ^ (d >> 1)); }
} // namespace mfsk
