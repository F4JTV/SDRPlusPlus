...
SDRPP_MOD_INFO{
    /* Name:            */ "navtex_decoder",
    /* Description:     */ "NAVTEX (SITOR-B / CCIR 476 FEC) decoder",
    /* Author:          */ "SDR++ NAVTEX",
    /* Version:         */ 1, 0, 0,
    /* Max instances    */ -1
};
...
class NavtexDecoderModule : public ModuleManager::Instance {
public:
    NavtexDecoderModule(std::string name) {
        this->name = name;
...
private:
    ...
    void createDecoder() {
        decoder = std::make_unique<NavtexDecoder>(name, vfo);
        decoder->setVFO(vfo);
    ...
    static void menuHandler(void* ctx) {
        NavtexDecoderModule* _this = (NavtexDecoderModule*)ctx;
        float menuWidth = ImGui::GetContentRegionAvail().x;

        if (!_this->enabled) { style::beginDisabled(); }

        // --- Mode (NAVTEX preset: SITOR-B 100/170, fixed) ---
        ImGui::LeftLabel("Mode");
        ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
        if (ImGui::Combo(CONCAT("##navtex_mode_", _this->name), &_this->modeIdx, NAVTEX_PRESET_LIST)) {
        ...
        ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
        if (ImGui::Combo(CONCAT("##navtex_sb_", _this->name), &_this->vfoMode, "USB\0LSB\0NFM\0")) {
            _this->applyVFOMode(_this->vfoMode, false);
        }

        // --- Band view: smoothed audio-passband spectrum with a marker at the
        //     NAVTEX centre (AF) and a shaded mark/space footprint. Click or
        //     drag to set the AF. Same widget as the RTTY/PSK/MFSK modules. ---
        if (_this->decoder) {
            ...
            if (nb > 0) {
                ...
                // Two tone markers (mark/space) at AF +/- shift/2, plus a faint
                // centre line, so the spacing reflects the 170 Hz shift.
                double halfShift = 170.0 * 0.5;
                float xm = f2x(afc + halfShift);
                float xs = f2x(afc - halfShift);
                dl->AddLine(ImVec2(xm, p0.y), ImVec2(xm, p0.y + h), cMarker, 2.0f);
                dl->AddLine(ImVec2(xs, p0.y), ImVec2(xs, p0.y + h), cMarker, 2.0f);
                float xe = f2x(afc);
                dl->AddLine(ImVec2(xe, p0.y), ImVec2(xe, p0.y + h), IM_COL32(120, 120, 120, 160), 1.0f);

                // Click / drag to set AF (disabled while AFC owns the AF).
                ImGui::InvisibleButton(CONCAT("##navtex_band_", _this->name), ImVec2(w, h));
        ...
        // --- AFC ---
        if (ImGui::Checkbox(CONCAT("AFC##navtex_afc_", _this->name), &_this->afcEnabled)) {
        ...
        if (ImGui::SliderFloat(CONCAT("##navtex_af_", _this->name), &_this->afFreq, 700.0f, 2500.0f, "%.0f Hz")) {
            _this->decoder->setAFFreq(_this->afFreq);
            config.acquire(); config.conf[_this->name]["afFreq"] = _this->afFreq; config.release(true);
        }
        ImGui::EndDisabled();
        ...
        // --- Squelch ---
        if (ImGui::Checkbox(CONCAT("Squelch##navtex_sq_en_", _this->name), &_this->squelchEnabled)) {
        ...
        ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
        if (ImGui::SliderFloat(CONCAT("##navtex_sq_lvl_", _this->name), &_this->squelchLevel, -100.0f, 0.0f, "%.1f dB")) {
        ...
        // --- Mode-specific UI (Reverse / status + decoded text) ---
    ...
    std::unique_ptr<NavtexDecoder> decoder;
...
MOD_EXPORT void _INIT_() {
    ...
    json def = json({});
    config.setPath(root + "/navtex_decoder_config.json");
    config.load(def);
    config.enableAutoSave();
}

MOD_EXPORT ModuleManager::Instance* _CREATE_INSTANCE_(std::string name) {
    return new NavtexDecoderModule(name);
}

MOD_EXPORT void _DELETE_INSTANCE_(void* instance) {
    delete (NavtexDecoderModule*)instance;
}
