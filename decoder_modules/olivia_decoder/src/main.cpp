...
SDRPP_MOD_INFO{
    /* Name:            */ "olivia_decoder",
    /* Description:     */ "Olivia MFSK decoder (4..64 tones, 125..2000 Hz, Jalocha/FLDIGI engine)",
    /* Author:          */ "F4JTV",
    /* Version:         */ 0, 1, 0,
    /* Max instances    */ -1
};
...
#define VFO_SAMPLE_RATE  (olivia::OLIVIA_SR)   // 8 kHz: SSB demod -> 8 kHz audio -> engine
#define VFO_BANDWIDTH    3000.0

class OliviaDecoderModule : public ModuleManager::Instance {
public:
    OliviaDecoderModule(std::string name) {
        this->name = name;
        ...
        modeIdx    = std::clamp(modeIdx, 0, olivia::OLIVIA_MODE_COUNT - 1);
        ...
        decoder = new olivia::OliviaDecoder(vfo->output);
        decoder->onChar = [this](char c) { onChar(c); };
        decoder->setVFOMode((olivia::VfoMode)vfoMode);
        applyVfoGeometry();
...
private:
    ...
    static void menuHandler(void* ctx) {
        OliviaDecoderModule* _this = (OliviaDecoderModule*)ctx;
        ...
        if (ImGui::Combo(CONCAT("##olivia_mode_", _this->name), &_this->modeIdx, modesStr())) {
            _this->modeIdx = std::clamp(_this->modeIdx, 0, olivia::OLIVIA_MODE_COUNT - 1);
        ...
        ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
        if (ImGui::Combo(CONCAT("##olivia_vfomode_", _this->name), &_this->vfoMode, "USB\0LSB\0")) {
            _this->vfoMode = std::clamp(_this->vfoMode, 0, 1);
            _this->decoder->setVFOMode((olivia::VfoMode)_this->vfoMode);
        ...
        ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
        if (ImGui::Combo(CONCAT("##olivia_snap_", _this->name), &_this->snapId, _this->snapIntervals.txt)) {
            if (_this->vfo) { _this->vfo->setSnapInterval(_this->snapIntervals.value(_this->snapId)); }
            _this->saveConfig();
        }

        // --- Band view: audio passband spectrum with the Olivia signal band
        ...
        //     drag to set the signal centre. ----------------------------------
        {
            ...
            ImGui::InvisibleButton(CONCAT("##olivia_band_", _this->name), ImVec2(w, h));
        ...
        ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
        if (ImGui::SliderFloat(CONCAT("##olivia_center_", _this->name), &_this->centerHz,
                               400.0f, 2400.0f, "%.0f Hz")) {
            _this->decoder->setAFFreq(_this->centerHz);
            _this->saveConfig();
        }
        if (ImGui::Button(CONCAT("Auto-center##olivia_ac_", _this->name), ImVec2(menuWidth, 0))) {
        ...
        if (ImGui::CollapsingHeader(CONCAT("Sync settings##olivia_synch_", _this->name))) {
            ImGui::LeftLabel("Margin");
            ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
            if (ImGui::SliderInt(CONCAT("##olivia_margin_", _this->name), &_this->syncMargin, 2, 16)) {
                _this->decoder->setSyncMargin(_this->syncMargin);
                _this->saveConfig();
            }
            ImGui::LeftLabel("Integration");
            ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
            if (ImGui::SliderInt(CONCAT("##olivia_integ_", _this->name), &_this->syncInteg, 2, 8)) {
                _this->decoder->setSyncInteg(_this->syncInteg);
                _this->saveConfig();
            }
            if (ImGui::Checkbox(CONCAT("8-bit extended chars##olivia_8bit_", _this->name), &_this->eightBit)) {
        ...
        if (ImGui::Checkbox(CONCAT("Squelch##olivia_sqen_", _this->name), &_this->squelchEnabled)) {
            _this->decoder->setSquelchEnabled(_this->squelchEnabled);
            _this->saveConfig();
        }
        ImGui::BeginDisabled(!_this->squelchEnabled);
        ImGui::SetNextItemWidth(menuWidth);
        if (ImGui::SliderFloat(CONCAT("##olivia_sql_", _this->name), &_this->squelchLevel,
                               olivia::OLIVIA_SQUELCH_MIN, olivia::OLIVIA_SQUELCH_MAX, "%.1f dB")) {
        ...
        ImGui::Text("Baud: %5.2f   %d tones / %d Hz",
                    ...
                    olivia::OLIVIA_MODES[_this->modeIdx].tones,
                    olivia::OLIVIA_MODES[_this->modeIdx].bw);

        // --- Decoded text -----------------------------------------------------
        ImGui::TextUnformatted("Decoded text:");
        {
            std::lock_guard<std::mutex> lck(_this->textMtx);
            ImGui::BeginChild(CONCAT("##olivia_txt_", _this->name), ImVec2(menuWidth, 200.0f), true,
        ...
        if (ImGui::Button(CONCAT("Clear##olivia_clr_", _this->name), ImVec2(menuWidth, 0))) {
            std::lock_guard<std::mutex> lck(_this->textMtx);
    ...
    static const char* modesStr() {
        static std::string s;
        if (s.empty()) {
            for (int i = 0; i < olivia::OLIVIA_MODE_COUNT; i++) {
                s += olivia::OLIVIA_MODES[i].name;
                s += '\0';
            }
            s += '\0';
        }
    ...
    olivia::OliviaDecoder* decoder = nullptr;

    OptionList<int, double> snapIntervals;

    // Settings
    int    vfoMode        = olivia::VFO_MODE_USB;
    int    modeIdx        = 10;     // OL 16-500
    int    snapId         = 2;      // 50 Hz
...
MOD_EXPORT void _INIT_() {
    ...
    json def = json({});
    config.setPath(root + "/olivia_decoder_config.json");
    config.load(def);
    config.enableAutoSave();
}

MOD_EXPORT ModuleManager::Instance* _CREATE_INSTANCE_(std::string name) {
    return new OliviaDecoderModule(name);
}

MOD_EXPORT void _DELETE_INSTANCE_(void* instance) {
    delete (OliviaDecoderModule*)instance;
}
