...
// Olivia decoder DSP chain (same structure as the proven mfsk_decoder):
//
//   VFO IQ (8 kHz) -> PowerSquelch -> SSB(USB/LSB) -> Handler sink -> OliviaModem
//
// The VFO is created at 8 kHz so the SSB demodulator emits 8 kHz real audio,
// which is exactly what the Jalocha Olivia engine expects (its symbol/FFT
...
namespace olivia {
    ...
    static constexpr double OLIVIA_SSB_BW = 3000.0;   // SSB channel bandwidth (Hz)

    class OliviaDecoder : public Decoder {
    public:
        OliviaDecoder(dsp::stream<dsp::complex_t>* in) {
            vfoInput = in;
        ...
        void buildDSP() {
            ...
            ssb.init(&squelch.out, dsp::demod::SSB<float>::USB, OLIVIA_SSB_BW, OLIVIA_SR,
                     100.0 / OLIVIA_SR, 5.0 / OLIVIA_SR);
        ...
        void setMode(int idx) override {
            idx = std::clamp(idx, 0, OLIVIA_MODE_COUNT - 1);
        ...
        void setSquelchLevel(float dB) override {
            squelchLevel = std::clamp(dB, OLIVIA_SQUELCH_MIN, OLIVIA_SQUELCH_MAX);
            squelch.setLevel(currentSquelchLevel());
        }
    ...
    private:
        ...
        float currentSquelchLevel() const {
            return squelchEnabled ? squelchLevel : OLIVIA_SQUELCH_MIN;
        }
        ...
        int     modeIdx        = 10;      // default OL 16-500 (index in OLIVIA_MODES)
