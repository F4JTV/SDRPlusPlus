// Decode a real off-air Olivia WAV (mono, 8 kHz, 16-bit). Scans candidate
...
static std::string decodeAt(const std::vector<double>& audio, int tones, int bw,
    ...
    printable = 0;
    for (char ch : out) if (ch >= 32 && ch < 127) printable++;
    return out;
}
...
int main(int argc, char** argv) {
    ...
    double bestScore = -1, bestCenter = 0; std::string best;
    // scan centre across the SSB passband
    for (double center = 500; center <= 2200; center += 25.0) {
        int pr; double snr;
        std::string txt = decodeAt(audio, tones, bw, center, pr, snr);
        double score = pr + snr;          // printable chars dominate, SNR breaks ties
