// Standalone Olivia round-trip validation harness for the Jalocha MFSK engine.
...
// Encode an ASCII string to Olivia audio (8 kHz, mono, normalised ~[-1,1]).
...
int main(int argc, char** argv) {
    ...
    Mode modes[] = {
        {"OL 4-250", 4, 250}, {"OL 8-250", 8, 250}, {"OL 8-500", 8, 500},
        {"OL 16-500", 16, 500}, {"OL 16-1000", 16, 1000},
        {"OL 32-1000", 32, 1000}, {"OL 64-2000", 64, 2000},
    };
    std::string msg = "CQ CQ DE TEST OLIVIA 73 ";
    ...
    for (auto& m : modes) {
        ...
        bool ok = got.find("OLIVIA") != std::string::npos;
        printf("%-12s %6.2fs audio  SNR=%5.1f  %s  \"%s\"\n",
               m.name, a.size() / SR, snr, ok ? "PASS" : "FAIL", got.c_str());
    }

    printf("\n=== OL 8-250 vs noise (target SNR in-band) ===\n");
