    ...
    static void menuHandler(void* ctx) {
        ...
        if (_this->decoder) {
            ...
            if (nb > 0) {
                ...
                // Mark / space marker lines, spaced by the RTTY shift, to be
                // aligned on the two signal peaks. Spacing tracks the mode.
                double shift = _this->decoder->getToneShift();
                ImU32 cCenter = IM_COL32(120, 120, 130, 120);
                float xc = f2x(afc);
                dl->AddLine(ImVec2(xc, p0.y), ImVec2(xc, p0.y + h), cCenter, 1.0f);
                if (shift > 0) {
                    float xMark  = f2x(afc + shift * 0.5);
                    float xSpace = f2x(afc - shift * 0.5);
                    dl->AddLine(ImVec2(xMark,  p0.y), ImVec2(xMark,  p0.y + h), cMarker, 2.0f);
                    dl->AddLine(ImVec2(xSpace, p0.y), ImVec2(xSpace, p0.y + h), cMarker, 2.0f);
                } else {
                    dl->AddLine(ImVec2(xc, p0.y), ImVec2(xc, p0.y + h), cMarker, 2.0f);
                }
