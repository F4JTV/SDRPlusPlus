...
// THOR decoder DSP chain (identical structure to the working mfsk_decoder):
//
//   VFO IQ -> PowerSquelch -> SSB(USB/LSB) or FM(NFM) -> Handler sink -> ThorModem
//
// CANONICAL SDR++ PATTERN (identical to m17 / radio / psk / mfsk decoders):
...
namespace thor {
    ...
    static constexpr double THOR_SSB_BW = 3000.0;   // SSB channel bandwidth (Hz)
    static constexpr double THOR_NFM_BW = 12500.0;  // NFM bandwidth (Hz)

    class THORDecoder : public Decoder {
    public:
        THORDecoder(dsp::stream<dsp::complex_t>* in) {
            vfoInput = in;
        ...
        void buildDSP() {
            ...
            ssb.init(&squelch.out, dsp::demod::SSB<float>::USB, THOR_SSB_BW, AUDIO_SR,
                     100.0 / AUDIO_SR, 5.0 / AUDIO_SR);
            nfm.init(&squelch.out, AUDIO_SR, THOR_NFM_BW, true);
        ...
        void setMode(int idx) override {
            idx = std::clamp(idx, 0, THOR_MODE_COUNT - 1);
        ...
        void setSquelchLevel(float dB) override {
            squelchLevel = std::clamp(dB, THOR_SQUELCH_MIN, THOR_SQUELCH_MAX);
    ...
    private:
        ...
        float currentSquelchLevel() const {
            return squelchEnabled ? squelchLevel : THOR_SQUELCH_MIN;
        }
        ...
        int     modeIdx        = 4;        // default THOR 11 (index in THOR_MODES)
