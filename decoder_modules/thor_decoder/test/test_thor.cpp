// Standalone THOR loopback test harness (no SDR++ runtime required).
//
//   generator (48 kHz) -> + AWGN -> ThorModem::process -> decoded text
//
...
#include "thor/modem.h"
#include "thor/generator.h"
#include <cstdio>
#include <cstring>
#include <cstdint>
#include <random>
...
int main(int argc, char** argv) {
    std::string msg = "CQ CQ DE TEST THOR 0123456789 the quick brown fox =";
    double af = 1500.0;
    ...
    printf("THOR loopback test  (message %zu chars, AF %.0f Hz)\n", msg.size(), af);
    printf("%-12s", "mode");
    for (double s : snrs) { if (s > 90) printf("  %6s", "clean"); else printf("  %5.0fdB", s); }
    printf("\n");

    int passClean = 0, total = 0;
    for (int m = 0; m < THOR_MODE_COUNT; m++) {
        if (onlyMode >= 0 && m != onlyMode) continue;
        ThorGenerator gen(m, af);
        std::vector<float> clean = gen.generate(msg);
        if (m == 4) writeWav("/home/claude/thor_work/thor11_clean.wav", clean, (int)AUDIO_SR);

        printf("%-12s", THOR_MODES[m].name);
        bool first = true;
        for (double snr : snrs) {
